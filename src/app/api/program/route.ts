...
import { weddingProgramSchema, reorderSchema } from '@/lib/validations/schemas';
...
export async function POST(request: NextRequest) {
  ...
  const parsed = weddingProgramSchema.safeParse(body);
  if (!parsed.success) {
    return jsonError(parsed.error.issues[0]?.message ?? 'Invalid program item.');
  }
...
export async function PATCH(request: NextRequest) {
  ...
  const updates = parsed.data.ids.map((id, index) =>
    client.from('wedding_program').update({ order_index: index }).eq('id', id)
  );
